...
import { Clock, CheckCircle, XCircle, Circle, ArrowLeft, ChevronLeft, ChevronRight } from 'lucide-react';

const ExamHistoryDetails = () => {
  ...
  // Flatten all answers from all sections into a single array
  const getAllAnswers = () => {
    if (!resultData || !resultData.sections) return [];
    let all = [];
    Object.values(resultData.sections).forEach(section => {
      if (Array.isArray(section.answers)) {
        all = all.concat(section.answers);
      }
    });
    return all;
  };
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="mb-8">
          <ol className="flex items-center space-x-2">
            <li>
              <Link to="/" className="text-gray-500 hover:text-lime-500">Home</Link>
            </li>
            <li><span className="text-gray-400 mx-2">/</span></li>
            <li>
              <Link to="/profile" className="text-gray-500 hover:text-lime-500">Exam History</Link>
            </li>
            <li><span className="text-gray-400 mx-2">/</span></li>
            <li><span className="text-lime-500">Result Details</span></li>
          </ol>
        </nav>

        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <h1 className="text-2xl font-bold mb-6">Tên bài test: {resultData.exam_title}</h1>
